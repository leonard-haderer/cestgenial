...
def enrichir_crises():
    ...
    for col in ['latitude', 'longitude', 'accessibilite', 'intensite', 'source']:
    ...
    for idx, crise in df.iterrows():
        ...
        # Ne traite que les guerres
        if str(crise.get('type_crise', '')).strip().lower() != 'guerre':
            continue

        # Vérifie quels champs sont manquants
        champs_cibles = ['latitude', 'longitude', 'accessibilite', 'intensite', 'source']
        champs_manquants = [c for c in champs_cibles if est_manquant(crise.get(c))]

        # Si tout est déjà renseigné, passe à la suite
        if not champs_manquants:
            print(f"[{traites}/{total}] ✓ Guerre {idx+1} déjà complétée, passage au suivant...")
            continue
